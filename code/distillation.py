...
parser.add_argument('--task_name', type=str, default='distillation')
parser.add_argument('--click_type', type=str, default='random')
parser.add_argument('--multi_click', action='store_true', default=False)
parser.add_argument('--model_type', type=str, default='vit_b_ori')
parser.add_argument('--checkpoint', type=str, default='/media/wagnchogn/ssd_2t/lmy/FastSAM3D_copy/ckpt/sam_med3d.pth')
...
parser.add_argument('--step_size', type=list, default=[30, 45])
...
class BaseTrainer:
    def __init__(self, model, dataloaders, args):
        ...
        self.init_checkpoint(join(self.args.work_dir, self.args.task_name, 'sam_med3d.pth'))
    ...
    def init_checkpoint(self, ckp_path):
        ...
        if last_ckpt:
            pretrained_dict = last_ckpt['model_state_dict']
            model_dict = self.model.state_dict()

                # 1. 移除键名中的 "image_encoder." 前缀
            ...
            load_msg = self.model.load_state_dict(model_dict, strict=False)

                # 4. 重新初始化不匹配的 rel_pos 参数
            for name, param in self.model.named_parameters():
                if "rel_pos" in name and name not in compatible_dict:
                    print(f"重新初始化参数: {name}")
                        # 使用截断正态分布初始化（与 SAM 一致）
                    torch.nn.init.trunc_normal_(param, mean=0, std=0.02, a=-2, b=2)

            print(f"权重加载信息 - 缺失的键: {load_msg.missing_keys}")
            print(f"权重加载信息 - 意外的键: {load_msg.unexpected_keys}")
