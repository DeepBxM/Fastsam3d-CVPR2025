...
class Dataset_Union_ALL(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if sitk_image.GetSpacing() != sitk_label.GetSpacing():
            # 选择一个统一的spacing（这里使用label的spacing）
            reference_spacing = sitk_label.GetSpacing()

            # 对图像进行重采样
            resample = sitk.ResampleImageFilter()
            resample.SetReferenceImage(sitk_label)
            resample.SetSize(sitk_label.GetSize())
            resample.SetOutputSpacing(reference_spacing)
            resample.SetOutputDirection(sitk_label.GetDirection())
            resample.SetOutputOrigin(sitk_label.GetOrigin())
            resample.SetTransform(sitk.Transform())
            resample.SetDefaultPixelValue(sitk_image.GetPixelIDValue())
            resample.SetInterpolator(sitk.sitkLinear)  # 对于图像使用线性插值
